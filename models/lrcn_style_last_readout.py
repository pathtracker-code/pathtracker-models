...
class LRCNStyleLast(nn.Module):
    ...
    def __init__(self, batch_size, timesteps=8, filt_size=15, num_iter=50, exp_name='exp1', jacobian_penalty=False, grad_method='bptt'):
        ...
        '''
        super(LRCNStyleLast, self).__init__()
    ...
    def forward(self, x, epoch, itr, target, criterion, testmode=False):
        ...
        # apply readout here (reading out a label from last frame)
        output2=F.dropout(output2, p=0.5, training=self.training)
        output2=self.fc1(output2.view(self.batch_size, -1))
        output2=torch.squeeze(output2)
        output2=torch.sigmoid(output2.clone())
        loss = criterion(output2, target.float())

        jv_penalty = torch.tensor([1]).float().cuda() # no use, just to keep the code running with the old framework

        if testmode: return output2, states, loss
        return output2, jv_penalty, loss
